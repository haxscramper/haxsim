...
/*_________
 ...
 | dialogs | Copyright (c) 2014 - 2021 Guillaume Vareille
http://ysengrin.com
...
#        define _POSIX_C_SOURCE                                           \
            2 /* to accept POSIX 2 in old ANSI C standards */
#    endif
#endif

#if !defined(_WIN32) && (defined(__GNUC__) || defined(__clang__))
#    if !defined(_GNU_SOURCE)
#        define _GNU_SOURCE /* used only to resolve symbolic links. Can   \
                               be commented out */
#    endif
#endif
...
/******************************************************************************************************/
#ifdef _WIN32
/* if you want to use UTF-8 ( instead of the UTF-16/wchar_t functions at
the end of tinyfiledialogs.h ) Make sure your code is really prepared for
UTF-8 (on windows, functions like fopen() expect MBCS and not UTF-8) */
int tinyfd_winUtf8 = 1; /* on windows char strings can be 1:UTF-8(default)
                           or 0:MBCS */
...
int tinyfd_silent  = 1; /* 1 (default) or 0 : on unix, hide errors and
                           warnings from called dialogs */

/* Curses dialogs are difficult to use, on windows they are only ascii and
 * uses the unix backslah */
...
/* some systems don't set the environment variable DISPLAY even when a
graphic display is present.
set this to 1 to tell tinyfiledialogs to assume the existence of a graphic
display */
...
static char
    gTitle[] = "missing software! (we will try basic console input)";
...
#ifdef _MSC_VER
#    pragma warning(disable : 4996) /* allows usage of strncpy, strcpy,   \
                                       strcat, sprintf, fopen */
#    pragma warning(disable : 4100) /* allows usage of strncpy, strcpy,   \
                                       strcat, sprintf, fopen */
#    pragma warning(disable : 4706) /* allows usage of strncpy, strcpy,   \
                                       strcat, sprintf, fopen */
#endif
...
static void ensureFinalSlash(char* aioString) {
    if (aioString && strlen(aioString)) {
        ...
        if (strncmp(lastcar, SLASH, 1)) { strcat(lastcar, SLASH); }
    }
}
...
static void Hex2RGB(char const aHexRGB[8], unsigned char aoResultRGB[3]) {
    ...
    if (aoResultRGB) {
        if (aHexRGB) {
            ...
            aoResultRGB[2] = (unsigned char)strtoul(
                lColorChannel + 5, NULL, 16);
            lColorChannel[5] = '\0';
            aoResultRGB[1]   = (unsigned char)strtoul(
                lColorChannel + 3, NULL, 16);
            lColorChannel[3] = '\0';
            aoResultRGB[0]   = (unsigned char)strtoul(
                lColorChannel + 1, NULL, 16);
...
void tfd_replaceSubStr(
    ...
    if (aNewSubStr) { lNewSubStr = aNewSubStr; }
    p              = aSource;
    *aoDestination = '\0';
...
char const* tinyfd_getGlobalChar(
    char const*
        aCharVariableName) /* to be called from C# (you don't need this in C or C++) */
{
...
int tinyfd_getGlobalInt(
    char const*
        aIntVariableName) /* to be called from C# (you don't need this in C or C++) */
{
...
#ifdef _WIN32
static int powershellPresent(void) { /*only on vista and above (or
                                        installed on xp)*/
...
#    if !defined(__MINGW32__) || defined(__MINGW64_VERSION_MAJOR)
    ...
    if (hMod) {
        ...
        if (lFxPtr) {
            ...
            if (!lFxPtr(&lRovi)) { return lRovi.dwMajorVersion; }
        }
    }
#    endif
...
#        define WC_ERR_INVALID_CHARS                                      \
            0x00000000 /* 0x00000080 for MINGW maybe ? */
#    endif

static int sizeUtf16From8(char const* aUtf8string) {
    return MultiByteToWideChar(
        CP_UTF8, MB_ERR_INVALID_CHARS, aUtf8string, -1, NULL, 0);
}


static int sizeUtf16FromMbcs(char const* aMbcsString) {
    return MultiByteToWideChar(
        CP_ACP, MB_ERR_INVALID_CHARS, aMbcsString, -1, NULL, 0);
}
...
static int sizeMbcs(wchar_t const* aMbcsString) {
    int lRes = WideCharToMultiByte(
        CP_ACP, 0, aMbcsString, -1, NULL, 0, NULL, NULL);
...
wchar_t* tinyfd_mbcsTo16(char const* aMbcsString) {
    ...
    if (lSize) {
        ...
        lSize       = MultiByteToWideChar(
            CP_ACP, 0, aMbcsString, -1, lMbcsString, lSize);
    } else
...
char* tinyfd_utf16toMbcs(wchar_t const* aUtf16string) {
    ...
    if (lSize) {
        ...
        lSize       = WideCharToMultiByte(
            CP_ACP, 0, aUtf16string, -1, lMbcsString, lSize, NULL, NULL);
    } else
        strcpy(lMbcsString, "");
    return lMbcsString;
}
    ...
    wchar_t const* aSource) /* aoDestination and aSource can be the same */
{
    ...
    if (aSource) {
        ...
        if (!lTmp) { lTmp = wcsrchr(aSource, L'\\'); }
        if (lTmp) {
...
static void Hex2RGBW(
    ...
    if (aoResultRGB) {
        if (aHexRGB) {
            ...
            aoResultRGB[2] = (unsigned char)wcstoul(
                lColorChannel + 5, NULL, 16);
            lColorChannel[5] = '\0';
            aoResultRGB[1]   = (unsigned char)wcstoul(
                lColorChannel + 3, NULL, 16);
            lColorChannel[3] = '\0';
            aoResultRGB[0]   = (unsigned char)wcstoul(
                lColorChannel + 1, NULL, 16);
            /* printf("%d %d %d\n", aoResultRGB[0], aoResultRGB[1],
             * aoResultRGB[2]); */
        } else {
...
#    else
            swprintf(
                aoResultHexRGB, lPrintFormat, aRGB[0], aRGB[1], aRGB[2]);
#    endif
...
static char* ensureFilesExist(
    ...
    if (!aSourcePathsAndNames) { return NULL; }
                           ...
                           yesnocancel */
{
    ...
    if (quoteDetectedW(aTitle))
        return tinyfd_messageBoxW(
            L"INVALID TITLE WITH QUOTES",
            aMessage,
            aDialogType,
            aIconType,
            aDefaultButton);
    ...
    if (aDialogType && !wcscmp(L"okcancel", aDialogType)) {
        aCode += MB_OKCANCEL;
        if (!aDefaultButton) { aCode += MB_DEFBUTTON2; }
    ...
    lBoxReturnValue = MessageBoxW(
        GetForegroundWindow(), aMessage, aTitle, aCode);
    if (((aDialogType && wcscmp(L"okcancel", aDialogType) &&
    ...
    wchar_t const* aIconType) /* L"info" L"warning" L"error" */
{
    ...
    if (quoteDetectedW(aTitle))
        return tinyfd_notifyPopupW(
            L"INVALID TITLE WITH QUOTES", aMessage, aIconType);
    if (quoteDetectedW(aMessage))
        return tinyfd_notifyPopupW(
            aTitle, L"INVALID MESSAGE WITH QUOTES", aIconType);
    ...
    wchar_t const* aDefaultInput) /* L"" , if NULL it's a passwordBox */
{
    ...
    if (quoteDetectedW(aTitle))
        return tinyfd_inputBoxW(
            L"INVALID TITLE WITH QUOTES", aMessage, aDefaultInput);
    if (quoteDetectedW(aMessage))
        return tinyfd_inputBoxW(
            aTitle, L"INVALID MESSAGE WITH QUOTES", aDefaultInput);
    if (quoteDetectedW(aDefaultInput))
        return tinyfd_inputBoxW(
            aTitle, aMessage, L"INVALID DEFAULT_INPUT WITH QUOTES");
...
#    endif
    ...
    if (aDefaultInput) {
        ...
        wcscat(
            lDialogString, L"\"):If IsEmpty(result) then:WScript.Echo 0");
        wcscat(
            lDialogString, L":Else: WScript.Echo \"1\" & result : End If");
    } else {
        wcscpy(
            lDialogString,
            L"\n\
...
#    else
        lFile    = _wfopen(
            lDialogString, L"wt, ccs=UNICODE"); /*or ccs=UTF-16LE*/
#    endif
        fclose(lFile);
        ...
        wcscpy(
            lDialogString, L"cmd.exe /c mshta.exe \"%TEMP%\\tinyfd.hta\"");
    }
...
#    else
    if (!(lIn = _wfopen(
              lDialogString, L"rt, ccs=UNICODE"))) /*or ccs=UTF-16LE*/
#    endif
    {
    ...
    wchar_t const* aSingleFilterDescription) /* NULL or "image files" */
{
    ...
    for (i = 0; i < aNumOfFilterPatterns; i++) {
        if (quoteDetectedW(aFilterPatterns[i]))
            return tinyfd_saveFileDialogW(
                L"INVALID FILTER_PATTERN WITH QUOTES",
                aDefaultPathAndFile,
                0,
                NULL,
                NULL);
    ...
    if (aNumOfFilterPatterns > 0) {
        ...
        if (!(aSingleFilterDescription &&
              wcslen(aSingleFilterDescription))) {
                                  ...
                                  return */
{
    ...
    if (aAllowMultipleSelects) { ofn.Flags |= OFN_ALLOWMULTISELECT; }
    ...
    if (GetOpenFileNameW(&ofn) == 0) {
            ...
            if (i > MAX_MULTIPLE_FILES) {
                ...
                *p = L'\0';
                for (j = i; j >= 0; j--) {
                    p -= lLengths[j];
                    memmove(
                        p, lPointers[j], lLengths[j] * sizeof(wchar_t));
                    p--;
                    *p = L'\\';
                    p -= lBuffLen;
        ...
        if (lBuff)
            lBuff = (wchar_t*)(realloc(
                lBuff, (lBuffLen + 1) * sizeof(wchar_t)));
    }
...
static int __stdcall BrowseCallbackProcW(
    HWND   hwnd,
    UINT   uMsg,
    LPARAM lp,
    LPARAM pData) {
    ...
    wchar_t const* aDefaultPath) /* NULL or "" */
{
    ...
    if (quoteDetectedW(aTitle))
        return tinyfd_selectFolderDialogW(
            L"INVALID TITLE WITH QUOTES", aDefaultPath);
    if (quoteDetectedW(aDefaultPath))
        return tinyfd_selectFolderDialogW(
            aTitle, L"INVALID DEFAULT_PATH WITH QUOTES");
                           ...
                           yesnocancel */
{
    ...
    if (aMessage) {
        ...
        lMessage = (wchar_t*)malloc(
            (wcslen(lTmpWChar) + 1) * sizeof(wchar_t));
    ...
    lIntRetVal = tinyfd_messageBoxW(
        lTitle, lMessage, lDialogType, lIconType, aDefaultButton);
    ...
    char const* aSingleFilterDescription) /* NULL or "image files" */
{
    ...
    lFilterPatterns = (wchar_t**)malloc(
        aNumOfFilterPatterns * sizeof(wchar_t*));
    for (i = 0; i < aNumOfFilterPatterns; i++) {
        if (tinyfd_winUtf8)
            lTmpWChar = tinyfd_utf8to16(aFilterPatterns[i]);
        else
            lTmpWChar = tinyfd_mbcsTo16(aFilterPatterns[i]);
        lFilterPatterns[i] = (wchar_t*)malloc(
            (wcslen(lTmpWChar) + 1) * sizeof(wchar_t*));
    ...
    unsigned char       aoResultRGB[3])       /* { 0 , 0 , 0 } */
{
    ...
    lTmpWChar = tinyfd_colorChooserW(
        lTitle, lDefaultHexRGB, aDefaultRGB, aoResultRGB);
                           ...
                           yesnocancel */
{
    ...
    if (aDialogType && !strcmp("okcancel", aDialogType)) {
        if (!aDefaultButton) { strcat(lDialogString, "--defaultno "); }
        strcat(
            lDialogString,
    ...
    char const* aDefaultInput) /* "" , if NULL it's a passwordBox */
{
    ...
    if (!aDefaultInput) {
        strcat(
            lDialogString,
            " (sometimes nothing, no blink nor star, is shown in text "
            "field)");
    }
    ...
    char const* aDefaultPathAndFile) /* NULL or "" */
{
    ...
    if (!strchr(lPathAndFile, '/')) { strcat(lDialogString, "./"); }
    ...
    if (!strlen(lDialogString)) { return NULL; }
    return aoBuff;
}
...
static void writeUtf8(char const* aUtf8String) {
    ...
    (void)WriteConsoleW(
        lConsoleHandle, lTmpWChar, (DWORD)wcslen(lTmpWChar), &lNum, NULL);
}
                           ...
                           yesnocancel */
{
    ...
    if (tfd_quoteDetected(aMessage))
        return tinyfd_messageBox(
            aTitle,
            "INVALID MESSAGE WITH QUOTES",
            aDialogType,
            aIconType,
            aDefaultButton);

    if ((!tinyfd_forceConsole ||
         !(GetConsoleWindow() || dialogPresent())) &&
    ...
    char const* aIconType) /* "info" "warning" "error" */
{
    ...
    if (tfd_quoteDetected(aMessage))
        return tinyfd_notifyPopup(
            aTitle, "INVALID MESSAGE WITH QUOTES", aIconType);
    ...
    char const* aDefaultInput) /* "" , if NULL it's a passwordBox */
{
    ...
    if (tfd_quoteDetected(aMessage))
        return tinyfd_inputBox(
            aTitle, "INVALID MESSAGE WITH QUOTES", aDefaultInput);
    if (tfd_quoteDetected(aDefaultInput))
        return tinyfd_inputBox(
            aTitle, aMessage, "INVALID DEFAULT_INPUT WITH QUOTES");
    ...
    if ((!tinyfd_forceConsole ||
        ...
        if (tinyfd_winUtf8) {
            ...
            (void)ReadConsoleW(
                lConsoleHandle, lBuffW, MAX_PATH_OR_CMD, &lNum, NULL);
            if (!aDefaultInput) {
            ...
            if (!lEOF) { return NULL; }
            printf("\n");
            if (strchr(lBuff, 27)) { return NULL; }
            if (lBuff[strlen(lBuff) - 1] == '\n') {
                lBuff[strlen(lBuff) - 1] = '\0';
            }
            return lBuff;
    ...
    char const* aSingleFilterDescription) /* NULL or "image files" */
{
    ...
    for (i = 0; i < aNumOfFilterPatterns; i++) {
        if (tfd_quoteDetected(aFilterPatterns[i]))
            return tinyfd_saveFileDialog(
                "INVALID FILTER_PATTERN WITH QUOTES",
                aDefaultPathAndFile,
                0,
                NULL,
                NULL);
    ...
    if ((!tinyfd_forceConsole ||
        ...
        if (p)
            ...
            lBuff[0] = '\0';
        if (lPointerInputBox)
            strcpy(lPointerInputBox, lString); /* restore its previous
                                                  content to
                                                  tinyfd_inputBox */
        p = lBuff;
    }

    if (!p || !strlen(p)) { return NULL; }
    getPathWithoutFinalSlash(lString, p);
    if (strlen(lString) && !dirExists(lString)) { return NULL; }
    getLastName(lString, p);
    if (!filenameValid(lString)) { return NULL; }
    return p;
}
    ...
    char const* aDefaultPath) /* NULL or "" */
{
    ...
    if (tfd_quoteDetected(aDefaultPath))
        return tinyfd_selectFolderDialog(
            aTitle, "INVALID DEFAULT_PATH WITH QUOTES");
    ...
    if (!p || !strlen(p) || !dirExists(p)) { return NULL; }
    return p;
}
    ...
    unsigned char       aoResultRGB[3])       /* { 0 , 0 , 0 } */
{
    ...
    if (lPointerInputBox)
        strcpy(lString, lPointerInputBox); /* preserve the current content
                                              of tinyfd_inputBox */
    p = tinyfd_inputBox(
        aTitle, "Enter hex rgb color (i.e. #f5ca20)", lDefaultHexRGB);

    if (!p || (strlen(p) != 7) || (p[0] != '#')) { return NULL; }
    for (i = 1; i < 7; i++) {
        if (!isxdigit((int)p[i])) { return NULL; }
    }
    ...
    if (lPointerInputBox)
        strcpy(lPointerInputBox, lString); /* restore its previous content
                                              to tinyfd_inputBox */
...
int tfd_isDarwin(void) {
    ...
    if (lsIsDarwin < 0) {
        lsIsDarwin = !uname(&lUtsname) &&
                     !strcmp(lUtsname.sysname, "Darwin");
    }
    return lsIsDarwin;
}
...
#    ifdef _GNU_SOURCE /*to bypass this, just comment out "#define _GNU_SOURCE" at the top of the file*/
        if (lBuff[strlen(lBuff) - 1] == '\n')
            lBuff[strlen(lBuff) - 1] = '\0';
        lAllocatedCharString = realpath(
            lBuff, NULL); /*same as canonicalize_file_name*/
...
static char* getVersion(char const* aExecutable) /*version must be first
                                                    numeral*/
{
    static char lBuff[MAX_PATH_OR_CMD];
...
int isDialogVersionBetter09b(void) {
    ...
    if (!strlen(lDialogName) ||
        !(lVersion = (char*)getVersion(lDialogName)))
        return 0;
    /*lVersion = lTest ;*/
    /*printf("lVersion %s\n", lVersion);*/
...
static char* terminalName(void) {
    ...
    if (lTerminalName[0] == '*') {
        if (detectPresence("bash")) {
            ...
            strcpy(lShellName, "sh -c "); /*good enough for dialog &
                                             whiptail*/
        } else {
        ...
        if (tfd_isDarwin()) {
            if (*strcpy(lTerminalName, "/opt/X11/bin/xterm") &&
                ...
                strcat(
                    lTerminalName,
                    " -fa 'DejaVu Sans Mono' -fs 10 -title "
                    "tinyfiledialogs -e ");
            ...
            *strcpy(lTerminalName, "xterm") /*good (small without
                                               parameters)*/
            && detectPresence(lTerminalName)) {
            strcat(
                lTerminalName,
            ...
            *strcpy(lTerminalName, "terminator") /*good*/
            && detectPresence(lTerminalName)) {
            strcat(lTerminalName, " -x ");
            strcat(lTerminalName, lShellName);
        } else if (
            *strcpy(lTerminalName, "lxterminal") /*good*/
            && detectPresence(lTerminalName)) {
            strcat(lTerminalName, " -e ");
            strcat(lTerminalName, lShellName);
        } else if (
            *strcpy(lTerminalName, "konsole") /*good*/
            && detectPresence(lTerminalName)) {
            strcat(lTerminalName, " -e ");
            strcat(lTerminalName, lShellName);
        } else if (
            *strcpy(lTerminalName, "kterm") /*good*/
            && detectPresence(lTerminalName)) {
            strcat(lTerminalName, " -e ");
            strcat(lTerminalName, lShellName);
        } else if (
            *strcpy(lTerminalName, "tilix") /*good*/
            && detectPresence(lTerminalName)) {
            strcat(lTerminalName, " -e ");
            strcat(lTerminalName, lShellName);
        } else if (
            *strcpy(lTerminalName, "xfce4-terminal") /*good*/
            && detectPresence(lTerminalName)) {
            strcat(lTerminalName, " -x ");
            strcat(lTerminalName, lShellName);
        } else if (
            *strcpy(lTerminalName, "mate-terminal") /*good*/
            && detectPresence(lTerminalName)) {
            strcat(lTerminalName, " -x ");
            strcat(lTerminalName, lShellName);
        } else if (
            *strcpy(lTerminalName, "Eterm") /*good*/
            && detectPresence(lTerminalName)) {
            strcat(lTerminalName, " -e ");
            strcat(lTerminalName, lShellName);
        } else if (
            *strcpy(lTerminalName, "evilvte") /*good*/
            && detectPresence(lTerminalName)) {
            strcat(lTerminalName, " -e ");
            strcat(lTerminalName, lShellName);
        } else if (
            *strcpy(lTerminalName, "pterm") /*good (only letters)*/
            && detectPresence(lTerminalName)) {
            strcat(lTerminalName, " -e ");
            strcat(lTerminalName, lShellName);
        } else if (
            *strcpy(lTerminalName, "gnome-terminal") &&
            detectPresence(lTerminalName) &&
...
static int graphicMode(void) {
    return !(tinyfd_forceConsole &&
             (isTerminalRunning() || terminalName())) &&
           (getenvDISPLAY() ||
...
static int pactlPresent(void) {
    ...
    if (lPactlPresent < 0) { lPactlPresent = detectPresence("pactl"); }
...
static int playPresent() {
    ...
    if (lPlayPresent < 0) {
        lPlayPresent = detectPresence("sox"); /*if sox is present, play is
                                                 ready*/
    }
    return lPlayPresent;
}
...
static int beepPresent(void) {
    ...
    if (lBeepPresent < 0) { lBeepPresent = detectPresence("beep"); }
    return lBeepPresent;
}
...
static int xmessagePresent(void) {
    ...
    if (lXmessagePresent < 0) {
        lXmessagePresent = detectPresence("xmessage"); /*if not tty,not on
                                                          osxpath*/
    }
...
int tfd_qarmaPresent(void) {
    ...
    if (lQarmaPresent < 0) { lQarmaPresent = detectPresence("qarma"); }
...
int tfd_xpropPresent(void) {
    ...
    if (lXpropPresent < 0) { lXpropPresent = detectPresence("xprop"); }
...
int tfd_zenityPresent(void) {
    ...
    if (lZenityPresent < 0) { lZenityPresent = detectPresence("zenity"); }
...
int tfd_yadPresent(void) {
    ...
    if (lYadPresent < 0) { lYadPresent = detectPresence("yad"); }
...
void tinyfd_beep(void) {
    ...
    if (osascriptPresent()) {
        if (afplayPresent() >= 2) {
            strcpy(
                lDialogString, "afplay /System/Library/Sounds/Ping.aiff");
        } else {
            strcpy(
                lDialogString,
         ...
         * --frequency 440 --test sine > /dev/tty" ) ;*/
        strcpy(
            lDialogString,
    ...
    if ((lIn = popen(lDialogString, "r"))) { pclose(lIn); }

    if (pactlPresent()) { signal(SIGINT, SIG_DFL); }
}
                           ...
                           yesnocancel */
{
    ...
    if (osascriptPresent()) {
        ...
        strcat(
            lDialogString,
            " -e 'try' -e 'set {vButton} to {button returned} of ( "
            "display dialog \"");
        if (aMessage && strlen(aMessage)) {
        ...
        if (aDialogType && !strcmp("okcancel", aDialogType)) {
            ...
            strcat(
                lDialogString, "buttons {\"No\", \"Yes\", \"Cancel\"} ");
 ...
 -e 'else' -e 'return 0' -e 'end if' ");
        ...
        strcat(lDialogString, " \"");
        if (aMessage) { strcat(lDialogString, aMessage); }
        strcat(lDialogString, "\"");
        if (aDialogType && !strcmp("okcancel", aDialogType)) {
        ...
        tfd_zenityPresent() || tfd_matedialogPresent() ||
        ...
        if (tfd_zenityPresent()) {
            ...
            if ((tfd_zenity3Present() >= 4) && !getenv("SSH_TTY") &&
                ...
                strcat(
                    ...
                    "_NET_ACTIVE_WINDOW | cut -f 2)"); /* contribution:
                                                          Paul Rouget */
            }
        ...
        !gxmessagePresent() && !gmessagePresent() && !gdialogPresent() &&
...
if res is False :\n\tprint 0\n\
        ...
        gxmessagePresent() || gmessagePresent() ||
        ...
        xdialogPresent() || gdialogPresent() || dialogName() ||
        whiptailPresent()) {
        if (gdialogPresent()) {
            if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
...
tinyfdRes=$(cat /tmp/tinyfd.txt);echo $tinyfdBool$tinyfdRes");
                ...
                strcat(
                    lDialogString,
                    "10 60 ) 2>&1;if [ $? = 0 ];then echo 1;else echo "
                    "0;fi");
            }
        } else {
            if (aDialogType && !strcmp("yesnocancel", aDialogType)) {
                ...
                strcat(
                    lDialogString, "10 60 >/dev/tty) 2>&1;if [ $? = 0 ];");
                if (lWasXterm) {
                    strcat(
                        lDialogString,
                        "then\n\techo 1\nelse\n\techo 0\nfi "
        ...
        if (aDialogType && !strcmp("yesno", aDialogType)) {
            ...
            strcat(
                lDialogString,
                "answer=$( while ! head -c 1 | grep -i [ny];do true "
                ";done);");
            strcat(
                lDialogString,
            ...
            strcat(
                lDialogString,
                "answer=$( while ! head -c 1 | grep -i [oc];do true "
                ";done);");
            strcat(
                lDialogString,
            ...
            strcat(
                lDialogString,
                "answer=$( while ! head -c 1 | grep -i [nyc];do true "
                ";done);");
            strcat(
                lDialogString,
                "if echo \"$answer\" | grep -iq \"^y\";then\n\techo 1\n");
            ...
            strcat(
                lDialogString, "echo -n \"press enter to continue \"; ");
            strcat(lDialogString, "stty sane -echo;");
            strcat(
                lDialogString,
        ...
        !isTerminalRunning() && pythonDbusPresent() &&
        ...
        !isTerminalRunning() && (perlPresent() >= 2) &&
        !strcmp("ok", aDialogType)) {
        if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
...
'org.freedesktop.Notifications');");
        ...
        sprintf(
            lDialogString + strlen(lDialogString),
        ...
        !isTerminalRunning() && notifysendPresent() &&
        !strcmp("ok", aDialogType)) {
        ...
        if (aTitle && strlen(aTitle)) { printf("\n%s\n", aTitle); }
    ...
    char const* aIconType) /* "info" "warning" "error" */
{
    ...
    if (osascriptPresent()) {
        ...
        /* zenity 2.32 & 3.14 has the notification but with a bug: it
         * doesnt return from it */
    ...
    char const* aDefaultInput) /* "" , if NULL it's a passwordBox */
{
    ...
    if (osascriptPresent()) {
        ...
        strcat(
            lDialogString,
            ");if [ $? = 0 ];then echo 1$szAnswer;else echo "
            "0$szAnswer;fi");
    } else if (
        tfd_zenityPresent() || tfd_matedialogPresent() ||
        tfd_shellementaryPresent() || tfd_qarmaPresent()) {
        ...
        !gdialogPresent() && !xdialogPresent() && tkinter3Present()) {
        if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
            strcpy(tinyfd_response, "python3-tkinter");
            return (char*)1;
        }
        ...
        !gdialogPresent() && !xdialogPresent() && tkinter2Present()) {
        if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
            strcpy(tinyfd_response, "python2-tkinter");
            return (char*)1;
        }
...
frontmost of process \\\"Python\\\" to true' ''');");
        ...
        gdialogPresent() || xdialogPresent() || dialogName() ||
        whiptailPresent()) {
        if (gdialogPresent()) {
            if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
                strcpy(tinyfd_response, "gdialog");
        ...
        if (!aDefaultInput) { strcat(lDialogString, "-s "); }
        ...
        !gWarningDisplayed && !isTerminalRunning() && !terminalName()) {
        gWarningDisplayed = 1;
        tinyfd_messageBox(gTitle, tinyfd_needs, "ok", "warning", 0);
        if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
        ...
        if (aTitle && strlen(aTitle)) { printf("\n%s\n", aTitle); }
        if (aMessage && strlen(aMessage)) { printf("\n%s\n", aMessage); }
    ...
    char const* aSingleFilterDescription) /* NULL or "image files" */
{
    ...
    if (osascriptPresent()) {
        ...
        strcat(
            lDialogString, " --file-selection --save --confirm-overwrite");
 ...
 frontmost of process \\\"Python\\\" to true' ''');");
        ...
        if (!xdialogPresent() && !gdialogPresent()) {
            ...
            strcat(
                lDialogString,
                "tab: focus | /: populate | spacebar: fill text field | "
                "ok: TEXT FIELD ONLY");
            strcat(lDialogString, "\" ");
        }
        ...
        if (strlen(lString) && !dirExists(lString)) { return NULL; }
        getLastName(lString, p);
        if (!strlen(lString)) { return NULL; }
        return p;
    }

    if (tinyfd_verbose) printf("lDialogString: %s\n", lDialogString);
    if (!(lIn = popen(lDialogString, "r"))) { return NULL; }
    while (fgets(lBuff, sizeof(lBuff), lIn) != NULL) {}
    pclose(lIn);
    if (lBuff[strlen(lBuff) - 1] == '\n') {
        lBuff[strlen(lBuff) - 1] = '\0';
    ...
    int         aAllowMultipleSelects)            /* 0 or 1 */
{
    ...
    if (osascriptPresent()) {
        ...
        if (aAllowMultipleSelects) {
            ...
            strcat(
                lDialogString, "-e 'set mystring to mystring & \"|\"' ");
            strcat(
                lDialogString,
        ...
        if ((aNumOfFilterPatterns > 1) ||
            ((aNumOfFilterPatterns == 1) /*test because poor osx
                                            behaviour*/
             && (aFilterPatterns[0][strlen(aFilterPatterns[0]) - 1] !=
                 '*'))) {
            strcat(lDialogString, "filetypes=(");
...
\n\tprint lFilesString[:-1]\n\"");
        ...
        if (p)
            ...
            lBuff[0] = '\0';
        if (lPointerInputBox)
            strcpy(lPointerInputBox, lDialogString); /* restore its
                                                        previous content to
                                                        tinyfd_inputBox */
        if (!fileExists(lBuff)) {
            free(lBuff);
            ...
            lBuff = (char*)(realloc(
                lBuff, (strlen(lBuff) + 1) * sizeof(char)));
        }
        return lBuff;
    }
    ...
    if (osascriptPresent()) {
        ...
        if (!osx9orBetter())
            strcat(
                ...
                " -e 'tell application \"System Events\"' -e 'Activate'");
        strcat(
            lDialogString, " -e 'try' -e 'POSIX path of ( choose folder ");
        if (aTitle && strlen(aTitle)) {
            strcat(lDialogString, "with prompt \"");
            strcat(lDialogString, aTitle);
            strcat(lDialogString, "\" ");
        ...
        if (aDefaultPath && strlen(aDefaultPath)) {
            if (aDefaultPath[0] != '/') { strcat(lDialogString, "$PWD/"); }
            strcat(lDialogString, "\"");
            strcat(lDialogString, aDefaultPath);
            strcat(lDialogString, "\"");
        } else {
...
frontmost of process \\\"Python\\\" to true' ''');");
    ...
    /* printf( "lBuff: %s\n" , lBuff ) ; */
    if (!strlen(lBuff) || !dirExists(lBuff)) { return NULL; }
    return lBuff;
}
...
to set mycolor to choose color default color {");
        ...
        tfd_zenity3Present() || tfd_matedialogPresent() ||
        tfd_shellementaryPresent() || tfd_qarmaPresent()) {
        lWasZenity3 = 1;
