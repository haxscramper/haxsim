...
template DEBUG_PRINT*(`type`: untyped, lv: untyped, fmt: untyped) {.dirty.} = 
  debug_print(`type`, __FILE__, __FUNCTION__, __LINE__, lv, fmt, ` __VA_ARGS__`)


template ASSERT*(cond: untyped) {.dirty.} = 
  if not((cond)):
    DEBUG_PRINT(F_ASSERT, 0, astToStr(cond))
  

template ERROR*(fmt: untyped) {.dirty.} = 
  DEBUG_PRINT(F_ERROR, 0, fmt, ` __VA_ARGS__`)

template WARN*(fmt: untyped) {.dirty.} = 
  ON_DEBUG(F_WARN, 0, fmt, ` __VA_ARGS__`)

template INFO*(lv: untyped, fmt: untyped) {.dirty.} = 
  ON_DEBUG(F_INFO, lv, fmt, ` __VA_ARGS__`)

template DEBUG_MSG*(lv: untyped, fmt: untyped) {.dirty.} = 
  ON_DEBUG(F_MSG, lv, fmt, ` __VA_ARGS__`)

template MSG*(fmt: untyped) {.dirty.} = 
